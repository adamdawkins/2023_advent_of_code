...
class RacerTest < Minitest::Test
  ...
  def test_races
    assert_equal([[71530, 940200]], @racer.races.map {|race| [race.time, race.record] })
  end

  def test_number_of_wins_per_race
    assert_equal([71503], @racer.number_of_wins_per_race)
  end

  def test_answer
    assert_equal(71503, @racer.answer)
  end
end
