...
class HandTest < Minitest::Test
  ...
  def test_joker_count
    assert_equal(2, Hand.new("QJJQ2").joker_count)
  end

  def test_counts_without_jokers
    assert_equal({"Q" => 2, "2" => 1} , Hand.new("QJJQ2").counts_without_jokers)
  end
...
class CamelCardsTest < Minitest::Test
  ...
  def test_order
    assert_equal(["32T3K",
                  ...
                  "T55J5",
                  "QQQJA",
                  "KTJJT"], @camel_cards.order.map(&:hand).map(&:to_s))
  end

  def test_total_winnings
    assert_equal(5905, @camel_cards.total_winnings)
  end
end
